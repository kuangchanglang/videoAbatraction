...
''' 
    @description judge if img1 and img2 are almost same image
    @param img1 
    @param img2
...
'''
    @description get binary image from input image
    @param img
    @return binary img
'''
def get_binary(img):
...
'''
    @description write black-white img to outputpath from input img
    @param img
    @param outputpath
    @return none
'''
...
'''
    @description judge if this frame has score board, we have set a label image and saved in target_img[0](global)
    @param frame 
